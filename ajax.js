...
function registerAjax(event) {
    const username = document.getElementById("new_user").value; // Get the username from the form
    const password = document.getElementById("new_pwd").value; // Get the password from the form
    let exits = 0;
    
    // Make a URL-encoded string for passing POST data:
    //const data = { 'new_user': username, 'new_pwd': password};
...
function invite() {
    ...
    fetch("displayInvites.php", {
        ...
        .then(function(response){
            ...
            let i = 0;
            while(i < jsonData.inviters.length) {
                //console.log(jsonData.multiple[i]);
                //console.log(jsonData.ids[i]);
                    const month = jsonData.months[i];
                    const day = jsonData.days[i];
                    const inviter = jsonData.inviters[i];
                    const id = jsonData.ids[i];
                    const name = jsonData.names[i];                
                $("#invites").append("Invite from " +inviter + ": " + name + " " + month + "/" +day +" <button class='accept' id='accept' value=" + id +">Accept</button><p>");
                i++;
            }
            const acceptbuttons = document.getElementsByClassName('accept');
            for ( var j in Object.keys( acceptbuttons ) ) {
                acceptbuttons[j].addEventListener("click", updateInvite, false);
            }
            //console.log("show events happened");
            updateCalendar();
...
function updateInvite(e) {
    const eventID = e.target.value;
    const data = {'eventID': eventID};
    fetch("updateInvite.php", {
            method: 'POST',
            body: JSON.stringify(data),
            headers: { 'content-type': 'application/json' }
        })
        ...
        .then(function(data){
            if(data.accepted){
            //console.log( "Event Successfully Accepted");
          
        } else {
            //console.log(`${data.message}`);
            //cosnole.log("fail to accept");
        }
        });
        showEvents();
        document.getElementById("invites").hidden = false;
...
document.getElementById("invite_btn").addEventListener("click", invite, false);
